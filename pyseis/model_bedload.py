...
def model_bedload(
    ...
    **kwargs
    ...
    This function calculates a seismic spectrum as predicted by the
    model of Tsai et al. (2012) for river bedload transport. It's
    based on the R implementation by Sophie Lagarde and Michael Dietze.

    Parameters:
    -----------
    gsd : array-like, optional
        Grain-size distribution function. Should be provided as a 2D
        array with two columns: grain-size class (in m) and
        weight/volume percentage per class.
    d_s : float, optional
    ...
    e_0 : float
        Exponent characterizing quality factor increase with
    ...
    **kwargs : dict, optional
        ...
        - log_lim : Limits of grain-size distribution function
        template. Default is (0.0001, 100).
        - log_length : Length of grain-size distribution function
        template. Default is 10000.
    ...
    i.e., s_s > 0.2. In such cases, the unadjusted version tends to
    underestimate seismic power.
    ...
    Examples:
    ---------
    >>> result = model_bedload(d_s=0.7, s_s=0.1, r_s=2650, q_s=0.001,
                            h_w=4, w_w=50, a_w=0.005, f=(0.1, 20), r_0=600,
                            f_0=1, q_0=20, e_0=0, v_0=1295, x_0=0.374,
                            n_0=1, res=100)
    ...
    if gsd is None:
        ...
        p_s = (
            1 / (2 * s) * (1 + np.cos(np.pi * (np.log(x_log) -
                                               np.log(d_s)) / s))
        ) / x_log
    ...
    t_s_c50 = np.exp(
        2.59e-2 * chi**4 + 8.94e-2 * chi**3 + 0.142 * chi**2 + 0.41 *
        chi - 3.14
    )
    ...
    r_3 = (0.65 - ((s_c / 2.83) * np.tanh(s_x - 4.6))) ** (1 +
                                                           ((3.5 - s_p) / 2.5))
    w_1 = r_3 * 10 ** (r_2 + r_1)
    ...
    def calculate_psd(f, x_b, v_c, v_u):
        ...
        if adjust:
            psd_f = np.sum(p_s * psd_raw * n_0**2 * np.diff(x_log,
                                                            prepend=x_log[0]))
        else:
            psd_f = np.sum(p_s * psd_raw * n_0**2)
        return psd_f

    z = np.array([calculate_psd(f, x, v, u) for f, x, v, u in
                  zip(f_i, x_b, v_c, v_u)])
